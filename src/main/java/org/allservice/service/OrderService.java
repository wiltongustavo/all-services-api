...
public class OrderService {
    ...
    @Transactional
    public OrderResponseDTO criarOrdemServico(@org.checkerframework.checker.nullness.qual.MonotonicNonNull OrderResponseDTO orderDTO) {
        // 1. Busca do cliente pelo clientId do DTO
        ClientEntity client = clientRepository.findById(orderDTO.id())
                .orElseThrow(() -> new ResourceNotFoundException("Cliente não encontrado com o ID: " + orderDTO.id()));
    ...
    @Transactional
    public OrderResponseDTO atualizarStatus(Long id, String novoStatus) {
        OrderEntity pedido = orderRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Ordem de serviço não encontrada com o ID: " + id));

        // Regra de negócio: não permite alterar ordens já canceladas ou concluídas
        if ("CANCELADO".equalsIgnoreCase(pedido.getStatus()) || "CONCLUIDO".equalsIgnoreCase(pedido.getStatus())) {
            throw new BusinessException("Não é possível alterar o status de uma ordem já finalizada ou cancelada!");
        }
    ...
    @Transactional(readOnly = true)
    public List<OrderResponseDTO> listAllOrderService() {
        List<OrderEntity> pedidos = orderRepository.findAll();
